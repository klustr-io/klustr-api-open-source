...
@Component
public class KafkaExperimentConsentListener {

    private static final Logger log = LoggerFactory.getLogger(KafkaExperimentConsentListener.class);

    private final Storage storage;
    private final WebhookProvider webhooks;

    public KafkaExperimentConsentListener(Storage storage,
                                          HydraApi hydra,
                                          UserLoginTrackingRepository logins,
                                          KafkaTemplate<UserExperimentConsent> kafka,
                                          WebhookProvider webhooks,
                                          @Value(value = "${consent.listener.kafka.consumer}") String groupId) {
        log.info("Starting kafka experiment listener");
    ...
    public void update(ConsumerRecord<String, UserExperimentConsent> record) {
        UserExperimentConsent payload = record.value();
        ...
        if (project.getApp() != null && project.getApp().getWebhooks() != null && project.getApp().getWebhooks().getEndpoints() != null) {
            project.getApp().getWebhooks().getEndpoints().forEach(hook -> {
                ...
                if (action.contains("delete")) {
                    payload.getScopes().forEach(x -> x.setStatus(UserConsentScope.Status.REVOKED));
                }
                if (payload.getStatus() == UserExperimentConsent.Status.REVOKED) {
                    payload.getScopes().forEach(x -> x.setStatus(UserConsentScope.Status.REVOKED));
                }
                String eventType = "user.experiment.consent." + action;
